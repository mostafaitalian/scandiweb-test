import {ADD_CART, ADD_PRODUCT_CART, INC_PRODUCT_QUANTITY, DEC_PRODUCT_QUANTITY, REMOVE_PRODUCT_CART, INC_PRODUCT_QUANTITY_NO_ATTR} from '../actions/Cart'
...
export default function cartReducer(state=[], action){
    ...
    switch (action.type) {
        case ADD_PRODUCT_CART:
            return [...state, {product:action.product, quantity:action.quantity, selectedAttrs:action.attrs}]
        case REMOVE_PRODUCT_CART:
            ...
            const cartProduct = state.filter(cartProduct=>{
                ...
                return (cartProduct.product.id === action.productId && checkArraysEqual(a, action.attrData))
        ...
        case INC_PRODUCT_QUANTITY:{            
            ...
            const {product,quantity,selectedAttrs} = cartProduct
            const incQuantity = quantity + 1;
            return [...arrayBefore, {product, quantity: incQuantity, selectedAttrs}, ...arrayAfter]
        }
        case INC_PRODUCT_QUANTITY_NO_ATTR:{            
            const cartProduct = state.filter(cartProduct=>cartProduct.product.id === action.productId)[0]
        ...
        case DEC_PRODUCT_QUANTITY:{
            const cartProduct = state.filter(cartProduct=>{
                console.log("selected2",cartProduct, cartProduct.selectedAttrs)
                const a = getSelectedAtt(cartProduct.product.id,cartProduct.selectedAttrs)
                console.log("seleced3", cartProduct.product.id === action.productId, action.attrData, checkArraysEqual(a, action.attrData) )
                return (cartProduct.product.id === action.productId && checkArraysEqual(a, action.attrData))
